...
exports.protect = catchAsync(async (req, res, next) => {
	...
	// GRANTS ACCESS TO PROTECTED ROUTE WHICH IS THE NEXT MIDDLE WARE
	req.user = currentUser;
	res.locals.user = currentUser;
	next();
});
// Only for rendered purpose, no errors
exports.isLoggedIn = async (req, res, next) => {
	if (req.cookies.jwt) {
		// 1 Verification / Validate token
		try {
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
	...
	// Generate the random token and send it back as email
...
exports.resetPassword = catchAsync(async (req, res, next) => {
	...
	// 2 If token not expired there is a user set a new pass
	if (!user) {
